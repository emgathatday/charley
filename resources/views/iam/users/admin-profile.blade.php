    ...
    <div class="settings-content col-12 col-lg-9">
        ...
        <div id="section-security" class="{{ $sectionClass('security') }}">
            <div class="card">
                ...
                <div class="sec-row"><div class="sec-row-left"><div class="sec-row-icon {{ $admin->mfa_enabled ? 'icon-tone-success' : 'icon-tone-warning' }}" id="tfa-icon"><svg class="icon"><use href="/assets/icons/sprite.svg#icon-active-sessions-svg-viewbox-0-0"></use></svg></div><div><div class="sec-row-label">Two-factor authentication</div><div class="sec-row-sub" id="tfa-sub">{{ $mfaLabel }} - strongly recommended for admin accounts</div></div></div><label class="switch" id="tfaToggle" title="Enable 2FA"><input id="tfaToggleInput" type="checkbox" @checked($admin->mfa_enabled) onchange="handle2FA(this.closest('.switch'))"><span class="slider"></span></label></div>
                <div class="sec-row"><div class="sec-row-left"><div class="sec-row-icon icon-tone-success"><svg class="icon"><use href="/assets/icons/sprite.svg#icon-penalty-history-3-actions-recorded"></use></svg></div><div><div class="sec-row-label">Current session</div><div class="sec-row-sub">{{ $currentSessionSummary }}</div></div></div><span class="{{ $mfaPill }}" id="mfaStatusPill">{{ $mfaLabel }}</span></div>
        ...
        <div class="modal-overlay" id="pwModal" onclick="if(event.target===this) closePasswordModal()">
            <div class="pw-modal admin-profile-modal">
                <div class="modal-head"><div class="modal-head-top"><div class="modal-icon icon-tone-success"><svg class="icon"><use href="/assets/icons/sprite.svg#icon-account-penalty-and-freeze-3"></use></svg></div><button class="modal-close" type="button" onclick="closePasswordModal()"><svg class="icon"><use href="/assets/icons/sprite.svg#icon-change-password-choose-a-strong"></use></svg></button></div><div class="modal-title">Change password</div><div class="modal-sub">Choose a strong password you haven't used before.</div></div>
                <div class="modal-body"><div class="pw-form-grid"><div class="pw-field"><label>Current password</label><div class="pw-field-wrap"><input type="password" id="currentPw" placeholder="Enter current password" autocomplete="current-password"><button class="pw-eye" type="button" onclick="togglePwVis('currentPw', this)"><svg class="icon"><use href="/assets/icons/sprite.svg#icon-input-type-checkbox-id"></use></svg></button></div></div><div class="pw-field"><label>New password</label><div class="pw-field-wrap"><input type="password" id="newPw" placeholder="At least 8 characters" autocomplete="new-password" oninput="checkStrength(this.value)"><button class="pw-eye" type="button" onclick="togglePwVis('newPw', this)"><svg class="icon"><use href="/assets/icons/sprite.svg#icon-input-type-checkbox-id"></use></svg></button></div><div class="pw-strength"><div class="pw-strength-bar" id="pwBar"></div></div><div class="pw-hint" id="pwHint">Use 8+ characters, a mix of letters, numbers &amp; symbols.</div></div><div class="pw-field"><label>Confirm new password</label><div class="pw-field-wrap"><input type="password" id="confirmPw" placeholder="Repeat new password" autocomplete="new-password"><button class="pw-eye" type="button" onclick="togglePwVis('confirmPw', this)"><svg class="icon"><use href="/assets/icons/sprite.svg#icon-input-type-checkbox-id"></use></svg></button></div></div></div></div>
        ...
        <div class="modal-overlay" id="tfaModal" onclick="if(event.target===this) close2FAModal()">
            <div class="modal admin-profile-modal">
                ...
                <div class="modal-body"><div class="modal-step active" id="tfa-step1"><div class="modal-step-label">Step 1 &mdash; Scan QR code</div><div class="qr-box"><div class="qr-placeholder" id="tfaQrPlaceholder"><img id="tfaQrImage" alt="Authenticator QR code" style="width:132px;height:132px;display:none;"></div><div class="qr-code-text">Open <strong>Google Authenticator</strong>, <strong>Authy</strong>, or any TOTP app and scan this code.</div><div class="qr-manual" id="tfaManualKey">Preparing setup...</div><div class="otp-hint" id="tfaProvisioningUri" style="word-break:break-all;"></div><div class="otp-hint" id="tfaSetupError" style="color:#B91C1C;"></div></div><div class="otp-hint">Can't scan? Enter the manual key above into your app.</div></div><div class="modal-step" id="tfa-step2"><div class="modal-step-label">Step 2 &mdash; Enter verification code</div><div class="otp-row">@for ($i = 0; $i < 6; $i++)<input class="otp-input" type="text" maxlength="1" oninput="otpMove(this, {{ $i }})" id="otp{{ $i }}">@endfor</div><div class="otp-hint" id="tfaModalHint">Enter the 6-digit code from your authenticator app.</div></div><div class="modal-step" id="tfa-step3"><div class="modal-success"><div class="modal-success-icon"><svg class="icon"><use href="/assets/icons/sprite.svg#icon-profile-verification-queue-5-svg"></use></svg></div><h3>2FA enabled successfully</h3><p>Your account is now protected with two-factor authentication.</p><div id="tfaRecoveryCodes" class="otp-hint" style="margin-top:12px;word-break:break-word;"></div></div></div></div>
        ...
        <div id="section-sessions" class="{{ $sectionClass('sessions') }}">
            <div class="card">
                ...
                @forelse ($sessions as $session)
                    ...
                    @endphp
                    <div class="sec-row">
                        <div class="sec-row-left"><div class="sec-row-icon {{ $isCurrentSession ? 'icon-tone-success' : 'icon-tone-muted' }}"><svg class="icon"><use href="/assets/icons/sprite.svg#icon-active-sessions-svg-viewbox-0-0"></use></svg></div><div><div class="sec-row-label">{{ $session->user_agent ?: 'Unknown browser' }}</div><div class="sec-row-sub">{{ $session->ip_address ?: 'Unknown IP' }} &middot; Last active {{ $lastActive }}</div></div></div>
                ...
                @empty
                    <div class="sec-row"><div class="sec-row-left"><div class="sec-row-icon icon-tone-muted"><svg class="icon"><use href="/assets/icons/sprite.svg#icon-active-sessions-svg-viewbox-0-0"></use></svg></div><div><div class="sec-row-label">No database sessions recorded</div><div class="sec-row-sub">This account has no persisted session rows to display.</div></div></div><span class="pill warn">Unavailable</span></div>
                @endforelse
            </div>
        </div>
