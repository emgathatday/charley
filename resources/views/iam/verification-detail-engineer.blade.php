        ...
        <div class="profile-head-left">
            ...
            <div>
                ...
                <div class="profile-tags">
                    <span class="meta-chip"><svg class="icon"><use href="/assets/icons/sprite.svg#icon-partner-management-path-d-m9-12"></use></svg>{{ $expertise }}</span>
                    <span class="meta-chip"><svg class="icon"><use href="/assets/icons/sprite.svg#icon-14-yrs-experience-p"></use></svg>{{ $years !== null ? $years.' yrs experience' : 'Experience not recorded' }}</span>
                    <span class="meta-chip"><svg class="icon"><use href="/assets/icons/sprite.svg#icon-james-carter-petrotechvn-com-path-d-m22-16-92v3a"></use></svg>{{ $email }}</span>
                    <span class="meta-chip"><svg class="icon"><use href="/assets/icons/sprite.svg#icon-84-90-123-4567-svg"></use></svg>{{ $phone }}</span>
                </div>
            </div>
        </div>
        <div class="profile-head-right"><div class="sla-chip"><svg class="icon"><use href="/assets/icons/sprite.svg#icon-user-k-habib-flagged-for"></use></svg>{{ $verificationRequest->sla['label'] }} - {{ $verificationRequest->sla['sub'] }}</div><div class="submitted-info">Application ID: VR-{{ $verificationRequest->id }}<br>Submitted {{ $submittedAt }}</div></div>
    </div>

    <div class="detail-grid verification-detail-grid">
        <div>
            <div class="section-card section-card-padded">
                <div class="section-head verification-section-head"><div class="section-title"><svg class="icon"><use href="/assets/icons/sprite.svg#icon-my-profile-path-d-m"></use></svg>Profile Details</div></div>
            ...
            <div class="section-card section-card-padded">
                <div class="section-head verification-section-head"><div class="section-title"><svg class="icon"><use href="/assets/icons/sprite.svg#icon-partner-management-path-d-m9-12"></use></svg>Work History</div><div class="section-sub">Self-reported, cross-checked against submitted documents</div></div>
            ...
            <div class="section-card section-card-padded">
                <div class="section-head verification-section-head"><div class="section-title"><svg class="icon"><use href="/assets/icons/sprite.svg#icon-company-registration-certificate-verified-10"></use></svg>Credentials &amp; Documents</div><div class="section-sub">{{ count($documents) }} files submitted</div></div>
                <div class="row row-cols-1 row-cols-md-2 g-3">
                    @forelse ($documents as $document)
                        <div class="col"><div class="doc-card"><div class="doc-thumb icon-tone-danger"><svg class="icon"><use href="/assets/icons/sprite.svg#icon-company-registration-certificate-verified-10"></use></svg></div><div><div class="doc-name">{{ $document['name'] }}</div><div class="doc-meta">{{ $document['mime_type'] ?? 'File' }}{{ $document['size'] ? ' - '.number_format($document['size'] / 1024, 1).' KB' : '' }}</div></div>@if ($document['url'])<a class="doc-open" href="{{ $document['url'] }}" target="_blank" rel="noopener"><svg class="icon"><use href="/assets/icons/sprite.svg#icon-input-type-checkbox-id"></use></svg></a>@endif</div></div>
                    @empty
            ...
            <div class="section-card section-card-padded section-card-flush">
                <div class="section-head verification-section-head"><div class="section-title"><svg class="icon"><use href="/assets/icons/sprite.svg#icon-profile-verification-queue-5-svg"></use></svg>Verification Checklist</div></div>
                <div class="check-list">
                    <div class="check-item"><div class="check-left"><div class="check-icon done"><svg class="icon"><use href="/assets/icons/sprite.svg#icon-create-user-svg-viewbox-0-0"></use></svg></div><div><div class="check-title">Applicant identity data loaded</div><div class="check-sub">Bound from users and engineer profile records.</div></div></div><span class="check-status done">Ready</span></div>
        ...
        <div class="decision-panel">
            ...
            <button class="decision-btn approve" type="button" onclick="submitVerificationAction('approve')"><svg class="icon"><use href="/assets/icons/sprite.svg#icon-create-user-svg-viewbox-0-0"></use></svg>Approve &amp; Verify</button>
            <button class="decision-btn inreview" type="button" onclick="openActionModal('inreview')"><svg class="icon"><use href="/assets/icons/sprite.svg#icon-9-verifications-exceeded-the-48h"></use></svg>In Review</button>
            ...
            <button class="decision-btn reject" type="button" onclick="openActionModal('reject')"><svg class="icon"><use href="/assets/icons/sprite.svg#icon-reject-application-internal-reviewer-note"></use></svg>Reject Application</button>
